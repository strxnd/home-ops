...
- External: `glitg.${SECRET_DOMAIN}` (the default Minecraft port is implied), after confirming the existing WAN TCP `25565` forward targets `192.168.20.12:25565`.
- Acceptance checklist: `https://glitg-checklist.${SECRET_DOMAIN}`.
...
| --- | --- | --- | --- | --- |
| Health | Flux | Run `flux get kustomizations -n games glitg-smp` and `flux get helmreleases -n games glitg-smp` | Both resources are `Ready` at the intended Git revision | Not verified after reintroduction |
| Health | Pod | Run `kubectl -n games get pods` | Minecraft and acceptance-checklist controllers are ready | Not verified after reintroduction |
| Health | LAN connection | Join `192.168.20.12:25565` | Vanilla client reaches the server and can join | Not verified |
| Health | External connection | Join `glitg.${SECRET_DOMAIN}` from outside the LAN | Server is reachable after WAN port forwarding without appending a port | Manual verification required |
...
| Config | Git authority | Edit a mounted plugin config, push, reconcile, then restart deployment | ConfigMap is copied into `/data/plugins/<plugin>` and rule changes take effect | Not verified after reintroduction |
| Cleanup | Plugin inventory | List `/data/plugins/*.jar` after restart | Only declared plugin JARs exist; no ConditionalEvents, Denizen, EpicSafePvp, ItemLimiter, DisableRiptide, ForceXaeroFairPlay, InstantVillagerRestock, or PlaceholderAPI JAR remains | Not verified after reintroduction |
